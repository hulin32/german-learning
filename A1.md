## 1 Meeting people

| German | English |
| ------ | ------- |
| wer    | who     |
| was    | what    |
| warum  | why     |
| wie    | how     |
| welche | which   |
| wo     | where   |
| wann   | when    |

---

Wie heißen Sie?– Ich heiße Emma.

Wie ist Ihr Name? – Mein Name ist Lisa.

Wie heißt du? – Ich heiße Lisa.

Wer bist du? – Ich bin Lisa.

Wer sind Sie? - Ich bin Lisa.

Wer ist das? – Das ist Emma.

---

Wie schreibt man das? (How do you write that?)

---

Nicos Tasche ist weg. (Nicos bag is gone.)

---

| ich   | du     | er/sie/es | Sie    |                                                                                              |
| ----- | ------ | --------- | ------ | -------------------------------------------------------------------------------------------- |
| komme | kommst | kommt     | kommen | Most verbs are conjugated in the present tense as this                                       |
| wohne | wohnst | wohnt     | wohnen |                                                                                              |
| lerne | lernst | lernt     | lernen |                                                                                              |
| mache | machst | macht     | machen |                                                                                              |
| heiße | heißt  | heißt     | heißen | When the verb stem ends in -s, -ß, -z or -x , the second-person singular form only adds a -t |
| bin   | bist   | ist       | sind   | irregular                                                                                    |

---

Plural:

1st person, wir heißen

2nd person, ihr heißt

3rd person, sie heißen

---

Conjugation of the verb haben

present tense:
| Person     | Singular      | Plural    | Formal    |
| ---------- | ------------- | --------- | --------- |
| 1st person | ich habe      | wir haben |           |
| 2nd person | du hast       | ihr habt  | Sie haben |
| 3rd person | er/sie/es hat | sie haben |           |
| ---------- | ------------- | --------- | --------- |
| 1st person | ich hatte      | wir hatten |           |
| 2nd person | du hattest       | ihr hattet  | Sie hatten |
| 3rd person | er/sie/es hatte | sie hatten |           |
| Formal     |               |           | Sie hatten |

---

Woher kommst du?
 – Ich komme aus Deutschland
---

Conjugation of the verb kommen

|            | Singular        | Plural     | Formal (singular und plural) |
| ---------- | --------------- | ---------- | ---------------------------- |
| 1st person | ich komme       | wir kommen |                              |
| 2nd person | du kommst       | ihr kommt  |                              |
| 3rd person | er/sie/es kommt | sie kommen | Sie kommen                   |

---

Conjugation of the verb arbeiten

|            | Singular           | Plural       | Formal (singular und plural) |
| ---------- | ------------------ | ------------ | ---------------------------- |
| 1st person | ich arbeite        | wir arbeiten |                              |
| 2nd person | du arbeitest       | ihr arbeitet |                              |
| 3rd person | er/sie/es arbeitet | sie arbeiten | Sie arbeiten                 |

---

Vocabulary:

1. https://learngerman.dw.com/en/ich-hei%C3%9Fe-emma/l-37262882/lv
2. https://learngerman.dw.com/en/das-ist-nico/l-37262923/lv
3. https://learngerman.dw.com/en/woher-kommst-du/l-37263828/lv
4. https://learngerman.dw.com/en/nico-hat-ein-problem/l-37265543/lv

Grammer:

[Conjugation: present tense (1)](https://learngerman.dw.com/en/conjugation-present-tense-1/l-37263828/gr-38320838)

[Conjugation: present tense (2)](https://learngerman.dw.com/en/conjugation-present-tense-2/l-37265543/gr-38321810)

[Conjugation: haben](https://learngerman.dw.com/en/conjugation-haben/l-37265543/gr-38310825)

---

## 2 Contact details

1-20:

| Number | Breakdown     | German       |
| ------ | ------------- | ------------ |
| 1      | -             | eins         |
| 2      | -             | zwei         |
| 3      | -             | drei         |
| 4      | -             | vier         |
| 5      | -             | fünf         |
| 6      | -             | sechs        |
| 7      | -             | sieben       |
| 8      | -             | acht         |
| 9      | -             | neun         |
| 10     | -             | zehn         |
| 11     | -             | elf          |
| 12     | -             | zwölf        |
| 13     | drei + zehn   | dreizehn     |
| 14     | vier + zehn   | vierzehn     |
| 15     | fünf + zehn   | fünfzehn     |
| 16     | sechs + zehn  | **sechzehn** |
| 17     | sieben + zehn | **siebzehn** |
| 18     | acht + zehn   | achtzehn     |
| 19     | neun + zehn   | neunzehn     |
| 20     | -             | zwanzig      |

21-99 的组合规则,
德语数字的“反序”组合是重点！记住公式：个位 + und + 十位:

- 21 = **ein**undzwanzig（字面：1 和 20）
- 34 = vierunddreißig（4 和 30）
- 67 = siebenundsechzig（7 和 60）

十位数的规律：

- 10: **zehn**
- 20: **zwanzig**
- 30: **dreißig**
- 40: vierzig
- 50: fünfzig
- 60: **sechzig**
- 70: **siebzig**
- 80: achtzig
- 90: neunzig

三位数及以上:

- 100 = (ein)hundert（“ein”可以省略）
- 200 = zweihundert
- 365 = dreihundertfünfundsechzig（300 + 65）
- 1000 = (ein)tausend
- 2024 = zweitausendvierundzwanzig

---

Vocabulary:

1. https://learngerman.dw.com/en/zahlen-von-1-bis-100/l-37265621/lv
2. https://learngerman.dw.com/en/wichtige-nummern/l-37269501/lv
3. http://learngerman.dw.com/en/adressen/l-37269671/lv
4. https://learngerman.dw.com/en/auf-dem-amt/l-37269629/lv

---

## 3 In company

The three articles(3 个冠词):

der(Masculine) Wein/Saft/Kaffee/Tee
die(feminine) Milch/Cola
das(neuter) Wasser/Bier

---

Vowel change: e to i

| Person       | Singular       | Plural    |
| ------------ | -------------- | --------- |
| 1st person   | ich esse       | wir essen |
| 2nd person   | du isst        | ihr esst  |
| 3rd person   | er/sie/es isst | sie essen |
| Formal (S/P) | Sie essen      | Sie essen |

| Person       | Singular        | Plural     |
| ------------ | --------------  | ---------- |
| 1st person   | ich nehme       | wir nehmen |
| 2nd person   | du nimmst       | ihr nehmt  |
| 3rd person   | er/sie/es nimmt | sie nehmen |
| Formal (S/P) | Sie nehmen      | Sie nehmen |

| Person       | Singular          | Plural       |
| ------------ | ----------------- | ------------ |
| 1st person   | ich spreche       | wir sprechen |
| 2nd person   | du sprichst       | ihr sprecht  |
| 3rd person   | er/sie/es spricht | sie sprechen |
| Formal (S/P) | Sie sprechen      | Sie sprechen |

---

How much is it?

- Wie viel kostet das?
- Was kostet das?
- Wie teuer ist das?

| Question-word/Interrogative adverb | Verb   | Subject     |
| ---------------------------------- | ------ | ----------- |
| Was                                | kostet | das Fleisch? |
| Wie viel                           | kostet | das Fleisch? |
| Wie viel                           | kostet | die Gurken? |
| Wie teuer                          | ist    | das Fleisch? |
| Wie teuer                          | sind   | die Gurken? |

---

Vocabulary:

1. https://learngerman.dw.com/en/was-machst-du-hier/l-37278679/lv
2. https://learngerman.dw.com/en/was-trinkst-du/l-37279418/lv
3. https://learngerman.dw.com/en/eine-pizza-bitte/l-37279261/lv
4. https://learngerman.dw.com/en/zahlen-bitte/l-37280348/lv

---

## 4 Around the world

---


Ich war schon mal in der Schweiz - I've been to Switzerland before.

Ich war noch nie in der Schweiz - I've never been to Switzerland.

Nächste Woche fahre ich nach München! - Next week I'm going to Munich!

---

Info: When we're talking about cities and countries without articles, we respond to questions beginning with "Wo …", with the preposition "in". In response to the question "Wohin …", we use "nach". In response to the question "Woher …", we use "aus".

---

1. sein present tense:

| Person     | Singular      | Plural   |
| ---------- | ------------- | -------- |
| 1st person | ich bin       | wir sind |
| 2nd person | du bist       | ihr seid |
| 3rd person | er/sie/es ist | sie sind |
| Formal     | Sie sind      | Sie sind |

2. sein past tense:

| Person     | Singular      | Plural   | Formal (singular und plural) |
| ---------- | ------------- | -------- | ---------------------------- |
| 1st person | ich war       | wir waren|                              |
| 2nd person | du warst      | ihr wart |                              |
| 3rd person | er/sie/es war | sie waren| Sie waren                    |

---

Woher kommst du?
 – Ich komme aus Deutschland
 
Kommst du aus Deutschland?
 – Ja, ich komme aus Deutschland.

Wie ist deine Telefonnummer?
 – Meine Telefonnummer ist 0176 …

Warst du schon mal in Berlin?
 – Nein, ich war noch nie in Berlin.

Warst du schon mal in Spanien?
 – Ja, ich war schon mal in Spanien.

Wo wohnst du?
 – Ich wohne in der Wagnergasse. 

C – Potsdam liegt in der Nähe von Berlin.

---

Welche Sprachen sprichst du?
- Ich spreche Deutsch und Englisch.

---


Vocabulary:

1. https://learngerman.dw.com/en/ich-war-schon-in-berlin/l-37325550/lv
2. https://learngerman.dw.com/en/wo-liegt-das/l-37337877/lv
3. https://learngerman.dw.com/en/in-europa/l-37328492/lv
4. https://learngerman.dw.com/en/andere-l%C3%A4nder/l-37337244/lv

---

## 5 Things

---

|          | Definite Article | Indefinite Article | negative |
|----------|------------------|--------------------|----------|
| Masculine| der Stift        | ein Stift          | kein Stift |
| Feminine | die Brille       | eine Brille        | keine Brille |
| Neuter   | das Buch         | ein Buch           | kein Buch |
| plural   | die Stifte       | Stifte             | keine Stifte |
| plural   | die Brillen      | Brillen            | keine Brillen |
| plural   | die Bücher       | Bücher             | keine Bücher |

|              | Singular          | Plural      |
|--------------|-------------------|-------------|
| 1st person   | ich lese          | wir lesen   |
| 2nd person   | du liest          | ihr lest    |
| 3rd person   | er/sie/es liest   | sie lesen   |
| Formal       |                   | Sie lesen   |

---

### 德语中有四个主要的格

| 格（Fälle） | 德语名称   | 主要功能与用途                                                                 |
|-------------|------------|--------------------------------------------------------------------------------|
| 主格(第一格)        | Nominativ  | 用于表示句子的主语，即谁或什么执行动作                                         |
| 属格(第二格)        | Genitiv    | 用于表示所有关系或所属关系，相当于英语中的“of”结构                             |
| 与格(第三格)        | Dativ      | 用于表示句子的间接宾语，即动作间接受益或影响的对象；也用于某些介词搭配         |
| 宾格(第四格)        | Akkusativ  | 用于表示句子的直接宾语，即动作直接作用的对象                                   |


### Nominativ(主格), Wer, Was

masculine: der/ein/kein

feminine: die/eine/keine

neuter: das/ein/kein

plural: die/ - /keine


### Genitiv(属格)

masculine: des/eines/keines

feminine: der/einer/keiner

neuter: des/eines/keines

plural: der/-/keiner

### Dativ(与格，间接宾语，宾语从句)

masculine: dem/einem/keinem

feminine: der/einer/keiner

neuter: dem/einem/keinem

plural: den/-/keinen


### Akkusativ(宾格), Wen, Was

masculine: den/einen/keinen (only this one is different when compared to Nominativ)

feminine: die/eine/keine

neuter: das/ein/kein

plural: die/ - /keine

---
das Adjektiv: Adjectives are used to describe people or things.

---
Vocabulary:

1. https://learngerman.dw.com/en/was-ist-das/l-37368285/lv
2. https://learngerman.dw.com/en/wem-geh%C3%B6rt-das/l-37372077/lv
3. https://learngerman.dw.com/en/ich-habe-kein/l-37382992/lv
4. https://learngerman.dw.com/en/das-auto-ist-rot/l-37401537/lv
---

## 6 Living

---

zu + Adjectives

---
Vocabulary:

1. https://learngerman.dw.com/en/so-wohne-ich/l-37425145/lv
2. https://learngerman.dw.com/en/meine-wohnung/l-37425763/lv
3. https://learngerman.dw.com/en/sofa-sessel-und-tisch/l-37438475/lv
4. https://learngerman.dw.com/en/unser-haus/l-37439097/lv
---

## 7 Days and times
---
Wie spät ist es? 

Wie viel Uhr ist es?

---
Times are often requested with the question word wann:

***Wann beginnt der Kurs?***

***Wann arbeitest du?***


To answer with a time:

use um, if you are talking about a specific time:
Der Kurs beginnt um neun.

use von … bis …, if you are talking about a period of time:
Ich arbeite von acht bis fünf.

---

| time  |  phrase                   |
|----------|---------------------------------|
| 8:00 Uhr | Es ist acht Uhr morgens.      |
| 10:00 Uhr | Es ist zehn Uhr vormittags.    |
| 12:00 Uhr | Es ist zwölf Uhr mittags.      |
| 15:00 Uhr | Es ist drei Uhr nachmittags.   |
| 20:00 Uhr | Es ist acht Uhr abends.       |
| 00:00 Uhr | Es ist zwölf Uhr nachts.      |

| Time          | Part of the Day |
|---------------|-----------------|
| 6:00 – 10:00  | Morgen          |
| 10:00 – 12:00 | Vormittag       |
| 12:00 – 14:00 | Mittag          |
| 14:00 – 18:00 | Nachmittag      |
| 18:00 – 22:00 | Abend           |
| 22:00 – 6:00  | Nacht           |

---
发音äu: 哦1

---
可分动词(trennbare Verben, ex: auf|stehen)

| 语法情况                            | 是否分开 | 例句                                         |
|-------------------------------------|--------|---------------------------------------------|
| 陈述句（现在时、过去时）                  | ✅ 分开  | Ich stehe um 6 Uhr auf.                    |
| 命令句（Imperativ）                     | ✅ 分开  | Steh auf!                                    |
| 情态动词 + 不定式（müssen, wollen 等） | ❌ 不分开 | Ich will nicht aufstehen.                   |
| 完成时（Perfekt, Plusquamperfekt）        | ❌ 不分开 | Ich bin um 6 Uhr aufgestanden.               |
| 从句（Nebensatz）                       | ❌ 不分开 | Ich weiß, dass du früh aufstehst.            |
---
frequency:

nie, fast nie, selten, （normalerweise, manchmal, oft, fast immer, immer

---
Separable verbs (1):

You can often tell if a verb is separable by its prefix.

- Verbs with certain prefixes, for example: ab-, an-, auf-, aus-, ein-, mit-, nach-, weg-, zu-  are always separable.
- In contrast, verbs with certain prefixes, for example:  be-, ent-, er-, ver-, zer- are not separable.
- Verbs with prefixes like durch-, über-, um-, unter- can be separable or inseparable.
---
Am Montag, Dienstag, Mittwoch, Donnerstag, Freitag, Samstag, Sonntag 
---

***The words dann and danach have the same meaning. It does not matter whether you use dann or danach.***

---
### Vowel change: a to ä:

- You already know verbs with the vowel change e to i. Another group of irregular verbs has a vowel change from a to ä. Again, these irregularities only affect the second- and third-person singular present.

- Ich fahre nach Berlin. Und wohin fährst du, Lisa? – Ich fahre nach München.

---
|              | Singular       | Plural      |
|--------------|----------------|-------------|
| 1st person   | ich fahre      | wir fahren  |
| 2nd person   | du fährst      | ihr fahrt   |
| 3rd person   | er/sie/es fährt| sie fahren  |
| Formal       |                | Sie fahren  |

The verbs schlafen, waschen and anfangen also have a vowel change from a to ä.



---
Vocabulary:

1. https://learngerman.dw.com/en/emmas-tag/l-37442425/lv
2. https://learngerman.dw.com/en/tageszeiten/l-37445884/lv
3. https://learngerman.dw.com/en/am-sonntag-koche-ich/l-37452061/lv
4. https://learngerman.dw.com/en/emmas-wochenende/l-37452629/lv
---

## 8. Appointments
 
---
描述时间: You say "kurz nach" or "kurz vor" if it's a matter of a couple of minutes past or before a specific time.

---
Time with prepositions

Use the question word Wann …? when asking about time. Answer using the prepositions um, am (= an + dem), im (= in + dem) or in.

 

__um__ + time:

Wann stehst du auf?
Ich stehe um sieben Uhr auf.

 

__am__ + day of the week:

Wann gehst du ins Kino?
Ich gehe am Freitag ins Kino.

 

__im__ + month or season:

Wann ist die Party?
Die Party ist im August.

Wann hast du Geburtstag?
Ich habe im Sommer Geburtstag.

 

__in__ + a later point in time (in … Minuten, Tagen, Wochen, Monaten, Jahren):

Wann beginnt der Kurs?
Der Kurs beginnt in 20 Minuten.

 

There are different ways to ask about periods of time.

If you want to know when something starts and ends you ask: Von wann … bis wann …? You answer with:

__von … bis___ …+ time or day of the week:

Von wann bis wann arbeitest du?
Ich arbeite von Montag bis Freitag von acht Uhr bis fünf Uhr.

 

If you only want to know when something begins, you ask: Ab wann …? You answer with:

__ab__ + time or day of the week

Ab wann hat das Restaurant geöffnet?
Das Restaurant hat ab neun Uhr geöffnet.

---
 Denkst du dran: remember 
---
序数词1-100的表达方式：
1. der Erste
2. der Zweite
3. der Dritte
4. der Vierte
5. der Fünfte
6. der Sechste
7. der Siebte
8. der Achte
9. der Neunte
10. der Zehnte
11. der Elfte
12. der Zwölfte
13. der Dreizehnte
14. der Vierzehnte
15. der Fünfzehnte
16. der Sechzehnte
17. der Siebzehnte
18. der Achtzehnte
19. der Neunzehnte
20. der Zwanzigste
22. der Zweiundzwanzigste
30. der Dreißigste
40. der Vierzigste
50. der Fünfzigste
60. der Sechzigste
70. der Siebzigste
80. der Achtzigste
90. der Neunzigste
100. der Hundertste

After prepositions of time, the ending of the ordinal number changes from

 -(s)te to -(s)ten:

Heute ist der siebzehnte Januar.

but:

Am siebzehnten Januar fahre ich nach Berlin.

 

When a date is given, some prepositions that come before the ordinal number also change:

von -> vom (= von + dem)

bis -> bis zum

Vom siebzehnten bis zum fünfundzwanzigsten Januar habe ich Urlaub.

ref: https://learngerman.dw.com/en/ordinal-numbers/l-37461035/gr-38304099

---

When you would like to know what the date tomorrow is, you ask:
1. "Der Wievielte ist morgen?"
2. "Welches Datum haben wir morgen?"

---
Januar - Februar - März - April - Mai - Juni - Juli - August - September – Oktober – November – Dezember

---

- diese Woche(this week) 
- nächste Woche(next week) 
- übernächste Woche(the week after next)

| Gender    | This       | Next       |
|-----------|------------|------------|
| Masculine(der) | diesen Monat | nächsten Monat |
| Feminine(die)  | diese Woche  | nächste Woche  |
| Neuter(das)    | dieses Jahr  | nächstes Jahr  |


---

| Pronoun    | können conjugation | müssen conjugation | wollen conjugation | möchten conjugation | sollen conjugation |
|------------|-------------------|-------------------|-------------------|---------------------|---------------------|
| ich        | kann              |  muss              | will                | möchte              | soll                |
| du         | kannst            |  musst             | willst              | möchtest            | sollst             |
| er/sie/es  | kann              |  muss              | will                | möchte              | soll               |
| wir        | können            |  müssen            | wollen              | möchten             | sollen             |
| ihr        | könnt             |  müsst             | wollt               | möchtet             | sollt              |
| sie/Sie    | können            |  müssen            | wollen              | möchten             | sollen             |

---



---
Valcabulary:

1. https://learngerman.dw.com/en/wie-sp%C3%A4t-ist-es/l-37453621/lv
2. https://learngerman.dw.com/en/hast-du-morgen-zeit/l-37461035/lv
3. https://learngerman.dw.com/en/wann-spielen-wir/l-37468614/lv
4. https://learngerman.dw.com/en/zu-sp%C3%A4t/l-37460778/lv
---

## 9 Working life

---
Possessive determiners (物主冠词)

| 人称   | 物主冠词 |
|-------|--------|
| ich   | mein   |
| du    | dein   |
| er/es | sein   |
| sie   | ihr    |
| wir   | unser  |
| ihr   | euer   |
| sie   | ihr    |
| Sie   | Ihr    |


单数时，物主冠词的变格与不定冠词的变格一致：
- 第一格和第四格时，若物主冠词位于阴性名词或者复数名词之后，词尾为-e。
- 第四格时，若物主冠词位于阳性名词之后，词尾为-en。

---
Two-case prepositions (1)

When describing where someone or something is, a preposition + dative is often used. 

![prepositions](images/prepositions-1.png)
![prepositions](images/prepositions-2.png)

---
Vocabulary:
1. https://learngerman.dw.com/en/ich-bin-lehrerin/l-37469204/lv
2. https://learngerman.dw.com/en/mein-beruf/l-37475138/lv
3. https://learngerman.dw.com/en/wo-ist-der-aufzug/l-37476734/lv
4. https://learngerman.dw.com/en/traumberufe/l-37477568/lv
---

## 10 Orientation

---
### Prepositions: bei, von,  in, an

Wo arbeiten Sie? – Ich arbeite bei …

When you say which company you work for, you use the preposition bei + the company name.

Ich arbeite bei Siemens / bei Aldi / bei Auto Müller / …

If the company name has an article, this is in the dative case.

Ich arbeite beim Bundesamt für Migration und Flüchtlinge. (beim = bei + dem)

Ich arbeite bei der Post.




Wo ist Potsdam? – Potsdam ist in der Nähe von …

When explaining where an unknown place is, you often name a place nearby that is larger and better-known. Then you use the expression in der Nähe von.

Potsdam ist in der Nähe von Berlin.

When the place has an article, this is in the dative case.

Das Kino ist in der Nähe vom Bahnhof. (vom = von + dem)

Das Museum ist in der Nähe von der Touristeninformation.


in der Wagnergasse 136

am(an + dem) Friedensplatz  22
---
können Sie mir helfen?

kann ich Sie etwas fragen?
---

über: about

überqueren: to cross

gehen: to go

---
Location: Wo …?

Two-case prepositions in / an / unter / über / auf / vor / hinter / neben / zwischen + *dative*

Direction: Wohin …?

Two-case prepositions in / an / unter / über / auf / vor / hinter / neben / zwischen + *accusative*

zu + dative

an + dative + vorbei

durch + accusative

---

The verb sein is irregular in the imperative:

Statement: Sie sind pünktlich.

Imperative sentence: Seien Sie pünktlich.


---

mit + dative

ohne + accusative

---

bei: at, with

nimmt: takes 

---
Vacabulary:
1. https://learngerman.dw.com/en/wo-ist-der-bahnhof/l-37594551/lv
2. https://learngerman.dw.com/en/an-der-ampel-links/l-37595076/lv
3. https://learngerman.dw.com/en/mit-bus-und-bahn/l-37595395/lv
4. https://learngerman.dw.com/en/im-b%C3%BCro/l-37595501/lv
---

## 11 Eating and leisure

---
Lebensmittel: food

---

| Form        | Symbol | gern (gladly) |
|-------------|--------|---------------|
| Positive    | +      | gern          |
| Comparative | ++     | lieber        |
| Superlative | +++    | am liebsten   |

| Form        | Symbol | gut (good)    |
|-------------|--------|---------------|
| Positive    | +      | gut           |
| Comparative | ++     | besser        |
| Superlative | +++    | am besten     |

| Form        | Symbol | viel (much/many) |
|-------------|--------|------------------|
| Positive    | +      | viel             |
| Comparative | ++     | mehr             |
| Superlative | +++    | am meisten       |


---

Question words: welch-

| Gender        | Nominative | Accusative |
|---------------|------------|------------|
| Masculine (der) | welcher   | welchen    |
| Feminine (die)  | welche    | welche     |
| Neuter (das)    | welches   | welches    |
| Plural (die)    | welche    | welche     |

---

If two people or things are different in a particular respect, then we use the comparative + als:

Tarek kocht besser *als* Sebastian.

Max putzt mehr *als* Inge.

---

When we want to compare two people or things that are the same in a particular respect, we use (genau)so + the basic form of the adjective + wie:

Sebastian kocht *so gut wie* Lisa.

Max arbeitet *genauso viel wie* Tarek.

---

Ich putze (nicht) gern das Bad. 

Ich koche (nicht) gern. 

Ich räume (nicht) gern die Wohnung auf. 

Ich räume (nicht) gern die Spülmaschine ein.

Ich räume (nicht) gern die Spülmaschine aus. 

Ich bereite (nicht) gern das Essen vor. 

Ich spüle (nicht) gern.

---

Verbs with prefixes like ab-, an-, auf-, aus-, ein-, mit-, nach-, weg- and zu- are separable. In a sentence without an additional verb, the prefix comes at the end:

Ich *wasche* lieber *ab*.

In combination with a modal verb, the separable verb comes at the end of the sentence in its infinitive form. The prefix is not separated.

Ich möchte lieber *abwaschen*.

---
frequency:
![frequency](images/frequency.png)

nie = niemals; oft = häufig; immer = ständig.

Ich hänge die Wäsche oft/regelmäßig/manchmal/selten/nie auf.

---

gehen + infinitive

| Person | gehen | Infinitive | Translation |
|--------|-------|------------|-------------|
| Ich    | gehe  | arbeiten   | I'm going to work tomorrow |
| Inge   | geht  | spazieren  | Inge goes for a walk in the park |
| Wir    | gehen | angeln     | We like to go fishing |


---
Valcabulary:
1. https://learngerman.dw.com/en/lebensmittel/l-37597045/lv 
2. https://learngerman.dw.com/en/ich-mag-nicht/l-37599970/lv
3. https://learngerman.dw.com/en/haushaltsarbeit/l-37631238/lv
4. https://learngerman.dw.com/en/was-macht-dir-spa%C3%9F/l-37630630/lv
---

## 12. Food

---

Was darf es sein? - What do you want?

Darf es sonst noch etwas sein?(Sonst noch etwas?) - Anything else?

Ist das alles?(Sind das alles?) - Is that all?

---

Sie wünschen? - Can I help you?; What can I do for you?

---
Valcabulary:
1. https://learngerman.dw.com/en/mengen-und-preise/l-37628421/lv 
2. https://learngerman.dw.com/en/was-darf-es-sein/l-37635620/lv
3. https://learngerman.dw.com/en/sonst-noch-etwas/l-37646611/lv
4. https://learngerman.dw.com/en/wie-viel-mehl/l-37647289/lv
---

## 13. Weather and holiday

---
### Past participles: regular

With regular verbs, the past participle usually begins with the prefix ge-; then comes the verb root, followed by a -t at the end.

| Infinitive   | Past participle |
|--------------|-----------------|
| machen       | ge-mach-t       |
| frühstücken  | ge-frühstück-t  |
| kochen       | ge-koch-t       |


If the verb root ends in -t or -d, an additional e is added. This extra e is also added after a cluster of consonants. Often, you will be able to identify when an e is necessary, as without it, the word would be hard to say.

| Infinitive | Past participle |
|------------|-----------------|
| arbeiten   | ge-arbeit-et    |
| öffnen     | ge-öffn-et      |


Some verbs make the past participle without ge-:

- Verbs which end in -ieren
- Verbs with inseparable prefixes, i.e. be-, ent-, er-, ver-, zer-

| Infinitive    | Past participle |
|---------------|-----------------|
| fotografieren | fotografier-t   |
| reservieren   | reservier-t     |
| besuchen      | besuch-t        |


---

### Past participles: irregular

| Infinitive    | Past participle |
|---------------|-----------------|
| sehen         | gesehen         |
| nehmen        | genommen        |
| finden        | gefunden        |


---

### Irregular comparative and superlative

| Positive | Comparative | Superlative |
|----------|-------------|-------------|
| viel     | mehr        | am meisten  |
| gut      | besser      | am besten   |
| gern     | lieber      | am liebsten |

### Regular comparative and superlative

To form the superlative, we put "am" in front of the adjective and add the ending "-sten". If the adjective ends in "-d", "-t", "-ß", "-s", "-z", "-sch", or "-x", then the superlative takes the ending "-esten".

| Positive | Comparative | Superlative    |
|----------|-------------|----------------|
| schlimm  | schlimmer   | am schlimmsten |
| klein    | kleiner     | am kleinsten   |
| süß      | süßer       | am süßesten    |

Comparative with words containing umlauts

| Positive | Comparative | Superlative    |
|----------|-------------|----------------|
| warm     | wärmer      | am wärmsten    |
| lang     | länger      | am längsten    |
| kalt     | kälter      | am kältesten   |
| kurz     | kürzer      | am kürzesten   |

---
Time: seasons

Im *der Frühling* ist es manchmal regnerisch.

Im *der Sommer* ist es warm und sonnig.

Im *der Herbst* ist es nass und windig.

Im *der Winter* ist es kalt und dunkel.

---

| Infinitive | Perfect tense with "sein" |
|------------|---------------------------|
| fliegen    | ich bin geflogen          |
| schwimmen  | ich bin geschwommen       |
| fahren     | ich bin gefahren          |
| gehen      | ich bin gegangen          |
| reisen     | ich bin gereist           |

---

完成时用“haben”还是“sein”？

表示位置变动或者状态变化的动词，完成时态需要用“sein”。

---
第二分词：带有前缀的动词（1）:

带有“be-”、“zer-”、“er-”、“ent-”、“ver-”等不可分前缀的动词在构成完成时态的时候，不需要“ge-”。

beginnen ‒> (hat) begonnen

verrühren ‒> (hat) verrührt

此外：以“-ieren”结尾的动词在构成完成时态的时候，也不需要“ge-”，永远以“-t”结尾。

studieren ‒> (hat) studiert

可分动词的完成时态构成规则如下：
可分前缀（ab-/an-/auf-/aus-/ein-/…）+ ge- + 词干 + -t / -en：auf-ge-räum-t

---

第二分词：不规则变化
即使在不规则情况中，第二分词也基本都以ge-开始。许多动词的词干会发生变化。第二分词的不规则情况通常都以-en结尾。

| 不定式   | 第二分词      |
|---------|-------------|
| fahren  | ge-fahr-en  |
| fliegen | ge-flog-en  |
| singen  | ge-sung-en  |
| bleiben | ge-blieb-en |


带有不可分前缀的动词在构成第二分词的时候不需要ge-：

| 不定式    | 第二分词     |
|----------|------------|
| beginnen | begonn-en  |
| vergessen| vergess-en |
| verlieren| verlor-en  |

还有一些不规则的第二分词有特殊的形式，这些也需要熟记。

举例如下：

| 不定式    | 第二分词     |
|----------|------------|
| essen    | ge-gess-en |
| gehen    | ge-gang-en |
| wissen   | ge-wuss-t  |
| verstehen| verstand-en|


第二分词：可分动词
构成完成时需要的第二分词通常以ge-开始。但可分动词的第二分词，-ge-位于前缀与词干之间。

规则动词：

| 不定式    | 第二分词     |
|----------|------------|
| aufräumen | auf-ge-räum-t |
| einkaufen | ein-ge-kauf-t |

不规则动词：

| 不定式    | 第二分词     |
|----------|------------|
| fernsehen | fern-ge-seh-en |
| mitschreiben | mit-ge-schrieb-en |

---
代词：es

---

---

Vacabulary:
1. https://learngerman.dw.com/en/wie-war-dein-urlaub/l-37648534/lv
2. https://learngerman.dw.com/en/jahreszeiten/l-37651758/lv
3. https://learngerman.dw.com/en/der-ausflug/l-37651657/lv
4. https://learngerman.dw.com/en/wie-wird-das-wetter/l-37653197/lv
---

## 14. Clothing

---
形容词词尾变化表（不定冠词后）
| 格 | 阳性 | 阴性 | 中性 |
|----------|------------|------------|------------|
| 主格 (Nominativ) | ein -*er* | eine -*e* | ein -*es* |
| 宾格 (Akkusativ) | einen -*en* | eine -*e* | ein -*es* |
| 与格 (Dativ) | einem -*en* | einer -*en* | einem -*en* |
| 属格 (Genitiv) | eines -*en* | einer -*en* | eines -*en* |

形容词词尾变化表（定冠词后）
| 格 | 阳性 | 阴性 | 中性 | 复数 |
|----------|------------|------------|------------|------------|
| 主格 (Nominativ) | der -*e* | die -*e* | das -*e* | die -*en* |
| 宾格 (Akkusativ) | den -*en* | die -*e* | das -*e* | die -*en* |


以-el 或者-er 结尾的形容词在变格的时候需要将e 省去：
- dunkel: Nico trägt einen dunklen Pullover.
- teuer: Der teure Pullover ist schön.

少数形容词（super、extra 和其它颜色形容词） 不需要变格：
- super: Das ist ein super Film.
- extra: Möchtest du einen extra Teller für den Salat?

### 物主冠词和否定冠词之后的形容词变格
带有物主冠词（如mein、dein、sein等）或者否定冠词（kein）的名词:
- 单数时，形容词变格按照不定冠词情况下的规则。
- 复数时，形容词变格按照定冠词情况下的规则。

ex:

    阳性：
    - kein schicker Pullover（第一格）
    - keinen schicken Pullover（第四格）

    阴性：
    - keine schicke Hose（第一格和第四格）

    中性：
    - kein schickes Hemd（第一格和第四格）

    复数：
    - keine schicken Schuhe（第一格和第四格）
---

疑问词welch-
| 格/性别 | 第一格 | 第四格 |
|---------|-------|-------|
| 阳性(der) | welcher | welchen |
| 阴性(die) | welche | welche |
| 中性(das) | welches | welches |
| 复数(die) | welche | welche |


指示冠词dies-
指示冠词用于特指某物，更加准确指明或者强调群体中的某一个。有时候人们会用手指出所提及的对象。

和疑问词welch- 一样，指示冠词dies- 的词尾也和定冠词保持一致。

| 格/性别 | 第一格 | 第四格 |
|---------|-------|-------|
| 阳性(der) | dieser | diesen |
| 阴性(die) | diese | diese |
| 中性(das) | dieses | dieses |
| 复数(die) | diese | diese |


Welcher Pullover ist am teuersten? – Dieser Pullover ist am teuersten.

Welchen Pullover findest du am schönsten? – Diesen Pullover finde ich am schönsten.

Welche Hose ist am billigsten? – Diese Hose ist am billigsten.

Welches Kleid möchten Sie? – Ich möchte dieses Kleid, bitte.

Welche Turnschuhe nehmen Sie? – Ich nehme diese Turnschuhe hier.

---

支配第三格宾语的动词（1）

人称代词也可以作第三格宾语。此时需要用到人称代词的第三格：

- ich -> mir
- du -> dir

---

zu + 形容词（2）
- zu +形容词可以用于表达对于事物的数量、大小或者程度等的不满意，即觉得不合适或者不可接受。


---
Valcabulary:
1. https://learngerman.dw.com/zh/das-ist-jetzt-modern/l-50300255/lv 
2. https://learngerman.dw.com/zh/mein-lieblingspulli/l-50289043/lv
3. https://learngerman.dw.com/en/das-passt-gut/l-37661740/lv
4. https://learngerman.dw.com/en/schick/l-37661765/lv
---

## 15. Family

---
第一格定冠词之后的形容词：
- 第一格单数情况下，定冠词之后的形容词词尾为：-e。
- 第一格复数情况下，定冠词之后的形容词词尾为：-en。

第四格定冠词之后的形容词：
- 在阴性单数和中性单数情况下，第四格定冠词之后的形容词变格词尾：-e。
- 在阳性和复数情况下，第四格定冠词之后的形容词变格词尾：-en。
---
人称代词第四格（1）

第一格的人称代词之前已经学过。部分人称代词的第一格和第四格是一样的：“sie”、“Sie”和“es”。“ich”、“du”和“er”在第四格要进行变化：

第一人称单数：第一格“ich”- 第四格“mich”

​​​​第二人称单数：第一格“du”- 第四格“dich”

第三人称阳性单数：第一格“er”- 第四格“ihn”

第三人称阴性单数：第一格“sie”- 第四格“sie”

第三人称中性单数：第一格“es”- 第四格“es”

第三人称复数：第一格“sie”- 第四格“sie”

第二人称单数/复数（正式）：第一格“Sie” - 第四格“Sie”
---
人称代词：第四格

| 第一格 | 第四格 |
|-------|-------|
| ich   | mich  |
| du    | dich  |
| er    | ihn   |
| sie   | sie   |
| es    | es    |
| wir   | uns   |
| ihr   | euch  |
| sie   | sie   |
| Sie   | Sie   |

---
介词：für:
- 介词 für 用于描述行为的接受对象、目标或者目的。这个介词支配第四格，因此在 für之后的代词或者名词要用第四格形式。

---



---
Vacabulary:
1. https://learngerman.dw.com/en/meine-familie/l-37661741/lv
2. https://learngerman.dw.com/zh/meine-eltern/l-50303864/lv
3. https://learngerman.dw.com/en/meine-tante/l-37664498/lv
4. https://learngerman.dw.com/en/mein-bruder/l-37664164/lv
---
## 16. Sports and exercise
---
时间状语：Wann...？
询问时间通常用疑问词 wann:

Wann spielt ihr Fußball?

 

回答时间点时……

具体的时刻用 **um**：

Das Fußballtraining beginnt **um** zwei Uhr.

时间段用**von … bis …**：

Wir spielen **von** zwei **bis** vier Uhr Fußball.

回答星期几时……

一次性的时间安排用 **am**：

**Am** Freitag spielen wir Fußball.

规律性的时间安排要用副词：

Freitags spielen wir Fußball. (= Jeden Freitag spielen wir Fußball.)

时间段用**von ... bis ...**：

Ich muss **von** Montag **bis** Freitag arbeiten.

回答月份和季节时……

具体的时间用 **im**：

**Im** Januar fahre ich Ski.       

**Im** Sommer gehe ich regelmäßig joggen.

时间段用**von … bis …**：

**Von** Oktober **bis** April hat das Schwimmbad geschlossen.

---

时间状语：Wie oft...？
询问重复性的事件或者活动用wie oft：

Wie oft trainiert ihr?

回答时通常可用……

指示冠词 jeden/jede/jedes:

Wir trainieren jeden Tag/jeden Freitag. (der Tag -> jeder Tag -> 第四格：jeden Tag)

Wir trainieren jede Woche. (die Woche -> jede Woche)

Wir trainieren jedes Wochenende. (das Wochenende -> jedes Wochenende)
 

频率词 (einmal, zweimal, dreimal ...) 和介词 pro:

Wir trainieren zweimal pro Woche.

---

时间状语：Wie lange...？
询问时间长度用 wie lange:

Wie lange trainierst du?
或者：
Wie lange dauert das Training?

 

回答时通常可用……

时间长度+ lang:

Ich trainiere eine halbe Stunde lang.

(单词lang 在这里也可以省略：Ich trainiere eine halbe Stunde.)

动词dauern 和时间长度：

Das Training dauert zwei Stunden.

---

比较（4）
不规则升级： nah/nahe*和 hoch

形容词nah(e) 和 hoch 在升级时不规则变化：

nah(e) < näher < am nächsten

hoch < höher < am höchsten

 

Yanis springt hoch.

Tarek springt höher.

Max springt am höchsten.


* 形容词nah und nahe 意思相同。它们只是一个单词的两种写法。

---

祈使句：非正式用语
非正式祈使句：单数

a) 无词尾形式

非正式祈使句有单数形式和复数形式两种。单数形式由第二人称单数现在时构成。人称代词和人称结尾-st需要省去。如果动词以-s、-ß、-z 或者 –x 结尾，则只需要省略-t。在非正式祈使句中，单数人称动词通常没有词尾。

 

(Du) mach(st) viel Sport. -> imperativ: Mach viel Sport!

(Du) geh(st) oft spazieren. -> imperativ: Geh oft spazieren!

(Du) putz(t) regelmäßig die Zähne. -> imperativ: Putz regelmäßig die Zähne!

(Du) iss(t) mehr Obst. -> imperativ: Iss mehr Obst!

变位时有从 a 到 ä 的变音的动词，在祈使句时不需要变音。

(Du) fähr(st) mit dem Fahrrad zur Arbeit. -> imperativ: Fahr mit dem Fahrrad zur Arbeit!

(Du) schläf(st) genug. -> imperativ: Schlaf genug!

 

b) 带有词尾-e的特殊形式

很多祈使句也可以在词尾加-e，通常只是表达风格的不同：

Mach jeden Tag Sport!
或者： 
Mache jeden Tag Sport!

第一种用法在口语中较为常见，第二种则常用于书面作品中。加上词尾-e之后，某些祈使句听起来会比较过时或者像是说教。

如果位于第二人称单数之后的动词带有词尾-est，那么其祈使句形式需要带-e。

 (Du) arbeite(st) nicht zu viel. -> Imperativ: Arbeite nicht zu viel!

(Du) öffne(st) das Fenster. -> Imperativ: Öffne das Fenster!

如果动词变位时有从e到i的换音（如essen -> Iss!），那么词尾不可带-e。

 

非正式祈使句：复数

当需要对一群人使用祈使句的时候，要用到它的复数形式。它由第二人称复数现在时构成，但是人称代词需要省略，动词词尾保留不变。

(Ihr) macht viel Sport. -> Imperativ: Macht viel Sport!

(Ihr) esst mehr Obst. -> Imperativ: Esst mehr Obst!

(Ihr) fahrt mit dem Fahrrad zur Arbeit. -> Imperativ: Fahrt mit dem Fahrrad zur Arbeit!

(Ihr) arbeitet nicht zu viel. -> Imperativ: Arbeitet nicht zu viel.

 

总结：祈使句

规则形式如下：

非正式用语（单数）	Trink(e) viel Wasser!
非正式用语（复数）	Trinkt viel Wasser!
正式用语	Trinken Sie viel Wasser!
 

动词sein形式是不规则的:

非正式用语（单数）	Sei fair!
非正式用语（复数）	Seid fair!
正式用语	Seien Sie fair!
 

德语语法术语：

der Imperativ: 祈使句是动词的一种特殊形式，用于表示请求、要求或者命令。

---

名词化
动词通常表达的是一种行为、一个过程或者一种状态。但动词也可以被名词化，通过在不定式之前加一个冠词，使其成为可独立使用的名词。名词化的动词均为中性。与其它名词一样，它的首个字母需要大写，需要变格。


示例：

grillen -> das Grillen

Yanis findet das Grillen wichtiger als das Training.

essen -> das Essen

Nico soll die Tabletten nach dem Essen nehmen.

schlafen gehen -> das Schlafengehen

Nico soll die Salbe vor dem Schlafengehen benutzen.

---

情态动词：含义
语法总结：情态动词的含义

情态动词通常需要第二个动词，即独立动词。情态动词描述了主语和其行为之间的关系，具体行为则是通过独立动词体现。

 

借助情态动词可以表达以下含义：

 

können：能力和可能性

Nico kann Deutsch sprechen.

（他学过德语，有能力说德语。）

Nico kann bei Inge wohnen.

（他有住在Inge那里的可能性。）

 

dürfen：许可

Emma darf eine Freundin besuchen.

（她的母亲允许了这件事。）

dürfen +否定：禁止

Emma darf keinen Alkohol trinken.

（她的父母不允许她喝酒，同时法律也禁止儿童饮酒。）

 

müssen：必要/强制

Emma muss die Hausaufgaben machen.

（如果她不做作业，她的老师会惩罚她。）

müssen +否定：不是必要的

Wir müssen nicht mehr trainieren.

（我们已经足够健康，不需要再锻炼。）

 

sollen：要求

Nico soll studieren.

（Nico的父母想要他上大学。）

 

wollen：（强烈的）愿望、目的

Lisa will heute Abend mit Nina ins Kino gehen.

（Lisa和Nina一起计划了晚上看电影。）

 

möchten：（礼貌地表达）愿望

Selma möchte als Architektin arbeiten.

（她有一个成为建筑师的愿望。）

---

情态动词：变位
语法总结：情态动词变位

对于大多数情态动词：

单数人称的时候需要更换元音。
第一人称和第三人称的时候，动词的形式相同。
以下为不同形态动词的变位一览：

 

können

单数：	 
- 第一人称	ich kann
- 第二人称	du kannst
- 第三人称	er/sie/es/man kann

复数：	 
- 第一人称	wir können
- 第二人称	ihr könnt
- 第三人称	sie können
- 正式用语	Sie können
 

dürfen

单数：	 
- 第一人称	ich darf
- 第二人称	du darfst
- 第三人称	er/sie/es/man darf

复数：	 
- 第一人称	wir dürfen
- 第二人称	ihr dürft
- 第三人称	sie dürfen
- 正式用语	Sie dürfen
 

müssen

单数：	 
- 第一人称	ich muss
- 第二人称	du musst
- 第三人称	er/sie/es/man muss

复数：	 
- 第一人称	wir müssen
- 第二人称	ihr müsst
- 第三人称	sie müssen
- 正式用语	Sie müssen
 

sollen

单数：	 
- 第一人称	ich soll
- 第二人称	du sollst
- 第三人称	er/sie/es/man soll

复数：	 
- 第一人称	wir sollen
- 第二人称	ihr sollt
- 第三人称	sie sollen
- 正式用语	Sie sollen
 

wollen

单数：	 
- 第一人称	ich will
- 第二人称	du willst
- 第三人称	er/sie/es will

复数：	 
- 第一人称	wir wollen
- 第二人称	ihr wollt
- 第三人称	sie wollen
- 正式用语	Sie wollen
 

möchten

单数：	 
- 第一人称	ich möchte
- 第二人称	du möchtest
- 第三人称	er/sie/es/man möchte

复数：	 
- 第一人称	wir möchten
- 第二人称	ihr möchtet
- 第三人称	sie möchten
- 正式用语	Sie möchten

---
Vacabulary:
1. https://learngerman.dw.com/en/von-kopf-bis-fu%C3%9F/l-37664951/lv
2. https://learngerman.dw.com/en/bist-du-fit/l-37668333/lv
3. https://learngerman.dw.com/en/fitness/l-37668815/lv
4. https://learngerman.dw.com/en/ist-das-gesund/l-37670365/lv
---

## 17. Health
---
情态动词：dürfen
禁烟标志禁烟标志
Hier darf man nicht rauchen.

在本次课程中，你学习了最后一个情态动词dürfen。dürfen 表示允许或有资格做某件事情。这个允许通常来自权威的一方，如父母，政府或者医嘱。

Sie dürfen Wasser oder Tee trinken.

Hier dürfen Sie zwei Stunden parken.

权威的一方也可以禁止某些东西。表达禁止的时候，需要用dürfen 加上否定词。

Sie dürfen keinen Kaffee trinken.

Hier dürfen Sie nicht parken.

 

变位：

和其它情态动词一样，dürfen在单数的时候需要更换元音。第一和第三人称的形式一样。

单数：	 
第一人称	ich darf
第二人称	du darfst
第三人称	er/sie/es/man darf
复数：	 
第一人称	wir dürfen
第二人称	ihr dürft
第三人称	sie dürfen
正式用语：	Sie dürfen
 

复习：müssen

请再次阅读带有“müssen”的句子，比较其用法：

动词 müssen 表示一种必要、强制或者责任，表示必须要做某事：

Er muss arbeiten.（他必须工作，因为他需要钱来生活。）

如果句子中否定了müssen，不意味着强制和禁止。它表示做某事没有必要，但是如果主语愿意的话，也可以做。

Er muss nicht arbeiten.（他很富裕，有足够的钱生活。但是他有可能去工作，因为他对工作有兴趣。）

---

现在完成时：助动词（W）

复习：现在完成时，用haben还是sein?:

大部分动词的现在完成时态由助动词haben构成。所有支配第四格宾语的动词，在构成现在完成时态的时候，都需要使用haben ：

Max und Tarek haben mich zum Arzt gebracht.

需要用sein 构成完成时态的有…

…表达向特定方向的移动或者地点的变换，如gehen、fahren、fliegen、schwimmen 和
reisen:

Wir sind sofort zum Arzt gefahren.

…描述状态变化或者人无法控制的过程，如einschlafen、aufwachen和 passieren。
Ich bin im Krankenhaus aufgewacht.

…动词sein、bleiben和werden：
Ich bin drei Tage im Krankenhaus geblieben.

---

第二分词
复习：第二分词

规则变化动词的第二分词以-(e)t结尾。

不规则变化动词通常以-en结尾。

大部分动词的第二分词都以ge-开始。部分动词在构成第二分词的时候没有ge-：

以-ieren 结尾的动词
带有不可分前缀的动词，如be-、ent-、er-、ver-、zer-
若是可分动词，则其第二分词中的-ge- 位于前缀和词干之间。

 

示例：规则变化动词

| 不定式 | 第二分词 |
|-------|---------|
| stürzen | ge-stürz-t |
| passieren | passier-t |
| aufwachen | auf-ge-wach-t |

示例：不规则变化动词

| 不定式 | 第二分词 |
|-------|---------|
| liegen | ge-leg-en |
| verstehen | verstand-en |
| mitnehmen | mit-ge-nomm-en |


---
Vacabulary:
1. https://learngerman.dw.com/en/geht-es-dir-gut/l-37778457/lv 
2. https://learngerman.dw.com/en/beim-arzt/l-37778169/lv
3. https://learngerman.dw.com/en/gute-besserung/l-37780308/lv
4. https://learngerman.dw.com/en/nehmen-sie/l-37782214/lv  
----
## 18. Home
---

形容词变格

**复习：形容词变格**

形容词可直接位于所描述的名词之前。此时它的词尾需要根据名词进行改变。这个变化叫做形容词变格。形容词变化一共有三类，取决于名词前的冠词：不定冠词、定冠词还是没有冠词？


**复习：不定冠词之后的形容词变格**

在第一格和第四格中，形容词词尾与定冠词词尾一致：

第一格	 
| 定冠词形式 | 不定冠词形式 |
|------------|--------------|
| der Garten | ein großer Garten |
| die Familie | eine große Familie |
| das Haus | ein großes Haus |
| die Geschäfte | große Geschäfte |

在第四格中，阳性冠词需要发生变化，形容词词尾也是同理。其它形式保持不变。

第四格	 
| 定冠词形式 | 不定冠词形式 |
|------------|--------------|
| den Garten | ein großer Garten |
| die Familie | eine große Familie |
| das Haus | ein großes Haus |
| die Geschäfte | große Geschäfte |
 
例外

以-el 或者 -er 结尾的形容词，在变位的时候其词尾前的e通常要去掉：
teuer: Wir hatten ein teures Auto.
少数形容词（super、extra 和一些颜色形容词） 不变格：
super: Das war ein super Essen.

---

形容词变格

**复习：定冠词之后的形容词变格**

Inge schenkt Nico ein gestreiftes Hemd. – Nico findet das gestreifte Hemd nicht so schön.

当名词和形容词之前有不定冠词时，其词尾（第一格和第四格）与定冠词词尾一致。

如果名词和形容词之前有定冠词，其词尾只为-e 或者 -en 。

第一格	 
| 定冠词形式 | 定冠词形式 |
|------------|--------------|
| der Garten | der große Garten |
| die Familie | die große Familie |
| das Haus | das große Haus |
| die Geschäfte | die großen Geschäfte |
 

第四格	 
| 定冠词形式 | 定冠词形式 |
|------------|--------------|
| den Garten | den großen Garten |
| die Familie | die große Familie |
| das Haus | das große Haus |
| die Geschäfte | die großen Geschäfte |
 

第四格中，阳性名词前的形容词词尾为-en。

复数名词前的形容词词尾也为-en。

---
形容词变格

**复习：物主冠词或者否定冠词之后的形容词变格**

形容词位于物主冠词（mein、dein、sein …）或者否定冠词（kein）之后：

单数时，形容词词尾变化按照不定冠词之后的规则。
复数时，形容词词尾变化按照定冠词之后的规则。
 
阳性：
kein großer Garten (第一格）
keinen großen Garten (第四格)

阴性：
keine große Familie (第一格和第四格)

中性：
kein großes Haus (第一格和第四格)

复数：
keine großen Geschäfte (第一格和第四格)

---

形容词词尾（1）

**第一格：**

在不定冠词之后，形容词词尾与定冠词一致（-er/-e/-es）。在复数时，形容词词尾为“-e”。

在定冠词之后，单数情况下的形容词词尾为“-e”，复数情况下词尾为“-en”。

单数：
ein grüner Papagei – der grüne Papagei
eine große Stadt – die große Stadt
ein kleines Dorf – das kleine Dorf
复数：
grüne Papageien – die grünen Papageien

第四格：

大部分形容词的词尾在第一格和第四格时是一样的。但是有一个例外：单数情况下的阳性名词前面，需要用词尾“-en”：
einen grünen Papagei – den grünen Papagei

---

形容词变格

**复习：形容词变格**

形容词可直接位于所描述的名词之前。此时它的词尾需要根据名词进行改变。这个变化叫做形容词变格。形容词变化一共有三类，取决于名词前的冠词：不定冠词、定冠词还是没有冠词？

 
复习：不定冠词之后的形容词变格

在第一格和第四格中，形容词词尾与定冠词词尾一致：

第一格	 
| 定冠词形式 | 不定冠词形式 |
|------------|--------------|
| der Garten | ein großer Garten |
| die Familie | eine große Familie |
| das Haus | ein großes Haus |
| die Geschäfte | große Geschäfte |
 

在第四格中，阳性冠词需要发生变化，形容词词尾也是同理。其它形式保持不变。

第四格	 
| 定冠词形式 | 不定冠词形式 |
|------------|--------------|
| den Garten | ein großer Garten |
| die Familie | eine große Familie |
| das Haus | ein großes Haus |
| die Geschäfte | große Geschäfte |
 

例外

以-el 或者 -er 结尾的形容词，在变位的时候其词尾前的e通常要去掉：
teuer: Wir hatten ein teures Auto.
少数形容词（super、extra 和一些颜色形容词） 不变格：
super: Das war ein super Essen.

---

形容词变格

**复习：定冠词之后的形容词变格**

Inge schenkt Nico ein gestreiftes Hemd. – Nico findet das gestreifte Hemd nicht so schön.

当名词和形容词之前有不定冠词时，其词尾（第一格和第四格）与定冠词词尾一致。

如果名词和形容词之前有定冠词，其词尾只为-e 或者 -en 。

第一格	 
| 定冠词形式 | 定冠词形式 |
|------------|--------------|
| der Garten | der große Garten |
| die Familie | die große Familie |
| das Haus | das große Haus |
| die Geschäfte | die großen Geschäfte |
 

第四格	 
| 定冠词形式 | 定冠词形式 |
|------------|--------------|
| den Garten | den großen Garten |
| die Familie | die große Familie |
| das Haus | das große Haus |
| die Geschäfte | die großen Geschäfte |
 

第四格中，阳性名词前的形容词词尾为-en。

复数名词前的形容词词尾也为-en。

---

形容词变格

**复习：物主冠词或者否定冠词之后的形容词变格**

形容词位于物主冠词（mein、dein、sein …）或者否定冠词（kein）之后：
- 单数时，形容词词尾变化按照不定冠词之后的规则。
- 复数时，形容词词尾变化按照定冠词之后的规则。
 
阳性：
- kein großer Garten (第一格）
- keinen großen Garten (第四格)

阴性：
- keine große Familie (第一格和第四格)

中性：
- kein großes Haus (第一格和第四格)

复数：
- keine großen Geschäfte (第一格和第四格)

---

人称代词：第三格
人称代词：第三格

人称代词不仅有第一格，也会以其它格出现。比如在特定介词或者动词之后是第四格。部分介词或者动词也支配第三格。

第一格：Vermisst du spanisches Essen?

第四格：Wir haben für dich Paella gekocht.

第三格：Schmeckt dir die Paella?

| 第一格 | 第四格 | 第三格 |
|-------|-------|-------|
| ich(我)   | mich  | mir   |
| du(你)    | dich  | dir   |
| er(他)    | ihn   | ihm   |
| sie(她)   | sie   | ihr   |
| es(它)    | es    | ihm   |
| wir(我们)   | uns   | uns   |
| ihr(你们)   | euch  | euch  |
| sie(他们)   | sie   | ihnen |
| Sie(您)   | Sie   | Ihnen |

---

支配第三格宾语的动词（2）
部分德语动词支配第三格宾语。第三格宾语指明行为的目的以及接受者，通常为人。

示例：

> Der junge Mann hilft der alten Frau.
>
> Er hilft ihr.

主语： der junge Mann = er

第三格宾语：der alten Frau = ihr

其它示例：

> Die Hose passt dem Mann.
>
> Sie passt ihm.

> Das Essen schmeckt den Kindern.
>
> Es schmeckt ihnen.

> Ihre Familie fehlt der Frau.
>
> Sie fehlt ihr.

> Das Zimmer gefällt der Studentin.
>
> Es gefällt ihr.

---

介词+第三格
**语法总结：支配第三格的介词**
 

部分介词只支配第三格。我们已经学过以下介词：

aus: Tarek kommt aus der Türkei.

nach: Nach dem Essen lerne ich Deutsch.

von: Das Haus ist in der Nähe vom Bahnhof. (vom = von + dem)

bei: Selma wohnt bei ihnen.

zu: Nico geht zum Deutschkurs. (zum = zu + dem)

mit: Nico fährt mit dem Bus.

ab: Tarek hat ab dem ersten Februar eine Woche Urlaub.

 

在回答Wo ...?的提问的时候，可变格介词之后支配第三格。

Inge和Nico坐在餐厅Marek的餐桌前，Tarek站在一旁Inge和Nico坐在餐厅Marek的餐桌前，Tarek站在一旁
in: Nico, Inge und Tarek sind im Restaurant. (im = in + dem)

an: Nico und Inge sitzen am Tisch. (am = an + dem)

unter: Nicos Füße sind unter dem Tisch.

über: Die Lampe hängt nicht über dem Tisch von Nico und Inge.

auf: Das Essen steht auf dem Tisch.

vor: Die Blumenvase steht vor dem Teller von Inge.

hinter: Das Kuchenregal steht hinter dem Stuhl von Inge.

neben: Nicos Messer liegt neben dem Teller.

zwischen: Der Orangensaft steht zwischen dem Teller und der Blumenvase.
---

当你想用“haben”表达一个愿望，可以说“Ich hätte gern ...”：
Ich hätte gern mein eigenes Geschäft.

如果是用“haben”回答一个假设性的问题，则需要去掉“gern”。例如：当你中彩票的时候，你会怎么样？
Ich hätte viel Geld.

---

第二虚拟式：愿望（1）
为了表达非现实的事情，需要用到动词的一个特殊形式，即第二虚拟式。比如可以用这个形式表达愿望。

真实：Ich habe keinen eigenen Fahrradladen.

愿望：Ich hätte gern einen eigenen Fahrradladen.


haben 的第二虚拟式变位如下：

Nico闭着双眼，旁边有一个幻想画面，是一家写着“Nicos Fahrradladen”的自行车店Nico闭着双眼，旁边有一个幻想画面，是一家写着“Nicos Fahrradladen”的自行车店

| 人称 | 单数 | 复数 |
|------|------|------|
| 第一人称 | ich hätte | wir hätten |
| 第二人称 | du hättest | ihr hättet |
| 第三人称 | er/sie/es/man hätte | sie hätten |
| 正式用语 | | Sie hätten |

德语语法术语：

der Konjunktiv II: 第二虚拟式是动词的一种形式，可以用于表达非现实的情况。比如可以用它表达愿望和非现实的场景，当然也可以用于非常礼貌地表达请求或者提供建议。

---

第二虚拟式：愿望（2）
当人们在表达想要的是什么的时候，可以用sein 的特殊形式，即第二虚拟式。

真实：Ich bin nicht verheiratet.

愿望：Ich wäre gern verheiratet.


sein 的第二虚拟式变位如下：

Inge表情愉悦，身旁有一个幻想画面，画面里是一名男子为一名女子戴上结婚戒指Inge表情愉悦，身旁有一个幻想画面，画面里是一名男子为一名女子戴上结婚戒指
| 人称 | 单数 | 复数 |
|------|------|------|
| 第一人称 | ich wäre | wir wären |
| 第二人称 | du wärst (du wärest)* | ihr wärt (ihr wäret)* |
| 第三人称 | er/sie/es/man wäre | sie wären |
| 正式用语 | | Sie wären |
* 这里存在两种变位形式，但是括号里的形式很少被使用。

---

第二虚拟式：愿望（3）
每个动词都可以构成第二虚拟式。但日常生活中，人们一般用haben、 sein和情态动词。其它动词则是用würde + 不定式。

真实：Selma besucht ihre Großeltern nicht.

愿望：Selma würde gern ihre Großeltern besuchen.

 

真实：Emma isst kein Eis.

愿望：Emma würde gern ein Eis essen.


würde 的第二虚拟式变位如下，句子中的第二个动词不需要变位：
 

| 人称 | 单数 | 复数 |
|------|------|------|
| 第一人称 | ich würde | wir würden |
| 第二人称 | du würdest | ihr würdet |
| 第三人称 | er/sie/es/man würde | sie würden |
| 正式用语 | | Sie würden |

---

Vacabulary:
1. https://learngerman.dw.com/zh/meine-heimat/l-50400079/lv
2. https://learngerman.dw.com/zh/leben-in-deutschland/l-50402839/lv
3. https://learngerman.dw.com/zh/anders-als-zu-hause/l-50413148/lv
4. https://learngerman.dw.com/zh/ich-tr%C3%A4ume-von/l-50413216/lv
---